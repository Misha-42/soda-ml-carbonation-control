...
EXPERIMENT_NAME = "B1_baseline_k1_rf_v1"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "B1_baseline_k1_rf_v1"
...
def load_dataset():
    ...
    # proxy-time
    df[TIME_COL] = df[DATE_COL] + pd.to_timedelta(df[SHIFT_COL].apply(shift_to_hours), unit="h")
...
def select_feature_columns(df: pd.DataFrame):
    feature_cols = [c for c in df.columns if c.startswith("k1_")]

    if not feature_cols:
        raise ValueError("Не найдены признаки k1_*")
    ...
    if not numeric_cols:
        raise ValueError("После отбора не осталось числовых признаков k1_*")

    return numeric_cols, dropped_non_numeric


def build_report_md(n_rows, n_features, train_size, test_size, metrics, dropped_non_numeric):
    lines = []
    ...
    lines.append(f"- Число признаков k1_*: {n_features}")
    lines.append(f"- Train size: {train_size}")
    lines.append(f"- Test size: {test_size}")
    lines.append(f"- Модель: Random Forest")
    ...
    lines.append("- Это первый baseline для B1 на merged lab+SCADA данных.")
    if dropped_non_numeric:
        lines.append(f"- Исключено нечисловых k1_* признаков: {len(dropped_non_numeric)}")
    else:
        lines.append("- Все отобранные k1_* признаки числовые.")
    return "\n".join(lines)
...
def main():
    ...
    feature_cols, dropped_non_numeric = select_feature_columns(df)
    ...
    if split_idx <= 0 or split_idx >= len(df):
        raise ValueError(f"Некорректный split_idx={split_idx} для размера датасета {len(df)}")
    ...
    print(f"[INFO] Число признаков k1_*: {len(feature_cols)}")
    ...
    pd.DataFrame({"feature": feature_cols}).to_csv(
        OUTPUT_DIR / "selected_k1_features.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    if dropped_non_numeric:
        pd.DataFrame({"feature": dropped_non_numeric}).to_csv(
            OUTPUT_DIR / "dropped_non_numeric_k1_features.csv",
            index=False,
            encoding="utf-8-sig",
        )
    ...
    joblib.dump(model, OUTPUT_DIR / "random_forest_B1.joblib")
    ...
    with open(OUTPUT_DIR / "summary.txt", "w", encoding="utf-8") as f:
        ...
        f.write(f"Число признаков k1_*: {len(feature_cols)}\n")
    ...
    print(f"  - {OUTPUT_DIR / 'selected_k1_features.csv'}")
