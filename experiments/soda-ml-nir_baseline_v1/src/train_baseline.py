...
def train_baseline_models() -> None:
    ...
    for experiment in experiments:
        ...
        model_filename = f"{experiment['name']}.joblib"
    ...
    metrics_df = save_metrics(experiment_results, reports_dir / "baseline_metrics.csv")
    save_experiments_summary(
        experiment_results,
        reports_dir / "experiments_summary.csv",
    )

    plot_metric_comparison(
        metrics_df,
        metric_name="mae",
        output_path=reports_dir / "mae_comparison.png",
    )
    plot_metric_comparison(
        metrics_df,
        metric_name="rmse",
        output_path=reports_dir / "rmse_comparison.png",
    )
    ...
    if rf_reference_model is not None:
        plot_feature_importance(
            ...
            output_path=reports_dir / "rf_feature_importance.png",
        )
    if xgb_reference_model is not None:
        plot_feature_importance(
            ...
            output_path=reports_dir / "xgb_feature_importance.png",
        )

    summary = build_summary(metrics_df)
    save_summary(summary, reports_dir / "baseline_report.md")

    print("Baseline эксперименты завершены.")
    print(f"Метрики сохранены: {reports_dir / 'baseline_metrics.csv'}")
    print(f"Сводка экспериментов сохранена: {reports_dir / 'experiments_summary.csv'}")
    print(f"Графики сохранены: {reports_dir}")
    print(f"Отчёт сохранён: {reports_dir / 'baseline_report.md'}")
